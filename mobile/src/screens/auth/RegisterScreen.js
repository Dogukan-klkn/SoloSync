    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        <View style={styles.card}>
          ...
          <Text style={styles.subtitle}>Platform'a katılın</Text>
          ...
          <View style={styles.roleRow}>
            {ROLES.map((role) => (
              <TouchableOpacity
                key={role.value}
                style={[styles.roleBtn, form.role === role.value && styles.roleBtnActive]}
                onPress={() => update('role', role.value)}
              >
                <Text style={[styles.roleBtnText, form.role === role.value && styles.roleBtnTextActive]}>
                  {role.label}
                </Text>
              </TouchableOpacity>
            ))}
...
const styles = StyleSheet.create({
  ...
  roleRow: { flexDirection: 'row', gap: 12, marginBottom: 20, marginTop: 4 },
  roleBtn: {
    flex: 1, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)',
    borderRadius: 12, paddingVertical: 14, alignItems: 'center',
  },
  roleBtnActive: { borderColor: '#0ea5e9', backgroundColor: 'rgba(14,165,233,0.15)' },
  roleBtnText: { color: '#94a3b8', fontWeight: '600' },
